// (All the type definitions, helpers, and fetchVip function remain exactly the same as the last correct version)
// ... keep the top part of your file identical ...
...
async function fetchVip(): Promise<StrapiItem<PropertyAttr>[]> {
  ...
  const res = await fetch(`${API}/api/properties?${params.toString()}`, { next: { revalidate } });
  if (!res.ok) {
    console.error("Failed to fetch VIP properties:", await res.text());
    return [];
  }
...
export default async function ExclusiveMandates() {
  ...
  // The ONLY change is this wrapper div with the red border for debugging
  return (
