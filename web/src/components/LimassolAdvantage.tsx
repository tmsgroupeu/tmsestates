...
function Counter({
  ...
  useEffect(() => {
    const controls = animate(from, to, {
      ...
      onUpdate(value) => {
        if (ref.current) {
          ref.current.textContent = `${prefix ?? ""}${value.toLocaleString(
            "en-US",
  ...
  return (
    <span
      ref={ref}
      className="text-xl font-semibold text-sky-300 md:text-2xl"
    />
  );
}
...
export default function LimassolAdvantage() {
  ...
  return (
    <section
      ...
      aria-label="Limassol market advantages"
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-sm text-slate-300/85">
            Anchored by a strong legal framework, international business
            ecosystem and lifestyle appeal, Limassol offers a rare blend of
        ...
        <div className="relative mt-2" ref={laneRef}>
          ...
          <div
            className="
              relative grid gap-3 rounded-3xl
              bg-slate-900/95 px-4 py-4
            ...
            {stats.map((stat, i) => (
              <div
                ...
                className={`
                  ...
                  hover:-translate-y-1 hover:ring-sky-400/40
        ...
        <p className="text-[10px] text-slate-500">
          Indicators are illustrative and can be aligned with your Strapi-backed
          insights in a later iteration.
        </p>
      </div>
    </section>
  );
