/* FULL REPLACEMENT: src/components/ai/AIChatWidget.tsx */
"use client";

import { useChat, type Message } from '@ai-sdk/react';
import { useState, useRef, useEffect } from 'react';
import { X, Send, Headset, User, Sparkles } from 'lucide-react'; 
...
export default function AIChatWidget() {
  ...
  const [showInvite, setShowInvite] = useState(false); // For the "Hello" bubble
  const [hideToggle, setHideToggle] = useState(false); // To hide button near footer
  ...
  // 2. Listen for "Open Chat" event from Footer
  useEffect(() => {
    const handleOpen = () => setIsOpen(true);
    window.addEventListener("open-ai-chat", handleOpen);
    
    // Also trigger invite bubble after 5s
  ...
  // 3. Hide toggle button when near footer
  useEffect(() => {
    const checkFooter = () => {
        const footer = document.getElementById('page-footer');
        if (!footer) return;
        const rect = footer.getBoundingClientRect();
        // If footer is visible in viewport, hide toggle
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                     ...
                     <p className="text-[10px] text-white/60">AI Specialist Online</p>
                  </div>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              {messages.length === 0 && (
                <div className="mt-12 text-center text-white/40 space-y-4 px-4">
                  ...
                  <p className="text-xs">I can scan our entire portfolio instantly. Try asking:</p>
                ...
                <div key={m.id} className={`mb-4 flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`flex flex-col gap-2 max-w-[85%]`}>
                    ...
                    {m.toolInvocations?.map((tool) => {
                      if (tool.state === 'result' && tool.toolName === 'show_property' && tool.result && !tool.result.error) {
                        return <div key={tool.toolCallId} className="animate-in fade-in slide-in-from-bottom-2 duration-500"><MiniPropertyCard data={tool.result} /></div>;
                      }
                      return <div key={tool.toolCallId} className="w-full h-24 bg-white/5 rounded-xl animate-pulse flex items-center justify-center border border-white/5"><span className="text-xs text-white/30">Searching...</span></div>;
                    })}
                  </div>
                </div>
              ))}
